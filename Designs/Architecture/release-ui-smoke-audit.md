...
required app builds, and the tests selected for the current change. Release UI
smoke auditing is separate from that gate and should not be added to the
normal task completion flow by default.
...
3. Prefer representative iPhone and Mac coverage when available. For iPad,
   include it when the changed surface uses adaptive navigation, sidebars, or
   split-view behavior.
4. Keep screenshots and findings in the audit report, not as committed test
   artifacts.
5. Group screenshots by device, orientation, and screen so the report can be
...
sideways, cropped, blank, obscured, or otherwise insufficient for human
review.
