...
check_swiftlint_environment() {
  local project_file="Normle.xcodeproj/project.pbxproj"
  local package_url="https://github.com/SimplyDanny/SwiftLintPlugins"
  ...
  if ! grep -q --fixed-strings "$package_url" "$project_file"; then
    record_failure "Normle.xcodeproj is missing the SwiftLintPlugins package dependency."
    record_next_step "Add $package_url to Normle.xcodeproj."
  fi
}
