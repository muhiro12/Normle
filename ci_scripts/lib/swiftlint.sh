...
ci_swiftlint_resolve_binary() {
  local repository_root=$1
  local project_path=${2:-Normle.xcodeproj}
  local source_packages_directory
  local package_cache_directory
  local swiftpm_cache_directory
  local swiftpm_config_directory
...
ci_swiftlint_run() {
  local repository_root=$1
  local mode=$2
  local project_path=${3:-Normle.xcodeproj}
  local empty_message
  local start_message
  ...
  swiftlint_binary=$(ci_swiftlint_resolve_binary "$repository_root" "$project_path")

  echo "$start_message"
  case "$mode" in
    format)
      "$swiftlint_binary" lint --quiet --no-cache --fix --format --force-exclude "${swift_files[@]}"
      ;;
    lint)
      "$swiftlint_binary" lint --quiet --no-cache --strict --force-exclude "${swift_files[@]}"
      ;;
  esac
  echo "$finish_message"
}
